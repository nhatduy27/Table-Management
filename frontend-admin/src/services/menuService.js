...
const menuService = {
  ...
  // Create item (hỗ trợ cả có và không có photos)
  createItem: async (itemData, photos = []) => {
    try {
      // Nếu có photos, gửi qua form-data
      if (photos && photos.length > 0) {
        ...
        const response = await adminApi.post("/menu/items", formData, {
          headers: {
        ...
        // Không có photos, gửi JSON như cũ
