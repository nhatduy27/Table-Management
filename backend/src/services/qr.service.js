  ...
  static async generatePDF(table, qrBuffer) {
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.fontSize(10)
           .font('Helvetica')
           .text('Point your camera at the QR code to view menu', { align: 'center' });
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        for (const table of tables) {
          ...
          // Generate QR code synchronously for this table
          const url = this.generateQRUrl(table.id, table.qr_token);
          QRCode.toBuffer(url, {
            errorCorrectionLevel: 'H',
            type: 'png',
        ...
        setTimeout(() => {
          doc.end();
        }, tables.length * 100); // Wait for all QR codes to be generated
