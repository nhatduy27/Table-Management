...
const TableForm = ({ table, onClose, onSuccess }) => {
  ...
  const predefinedLocations = ['Indoor', 'Outdoor', 'Patio', 'VIP Room', 'Terrace', 'Garden'];
  ...
  const validateForm = () => {
    ...
    if (!formData.table_number.trim()) {
      ...
      newErrors.table_number = 'Table number can only contain letters, numbers, hyphens and underscores';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response?.data?.errors) {
        ...
        setErrors({
          submit: err.response?.data?.message || 'Failed to save table'
        });
      }
    } finally {
      setLoading(false);
