...
import { X, Star, ShoppingBag, ChevronLeft, ChevronRight, User, ChevronLeftCircle, ChevronRightCircle } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="relative h-64 sm:h-72 bg-gray-100 flex-shrink-0">
          ...
          <button onClick={onClose} className="absolute top-4 right-4 bg-black/30 hover:bg-black/50 text-white p-2 rounded-full transition-colors backdrop-blur-md">
            <X size={20} />
          </button>
          {images.length > 1 && (
            <>
              <button onClick={(e) => { e.stopPropagation(); handlePrevImage(); }} className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white p-1.5 rounded-full shadow-lg text-gray-800"><ChevronLeft size={20} /></button>
              <button onClick={(e) => { e.stopPropagation(); handleNextImage(); }} className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white p-1.5 rounded-full shadow-lg text-gray-800"><ChevronRight size={20} /></button>
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          <div className="p-5 border-b">
            ...
            <div className="flex gap-6 mt-4 border-b border-gray-100">
              <button onClick={() => setActiveTab('details')} className={`pb-2 text-sm font-bold transition-colors relative ${activeTab === 'details' ? 'text-orange-600' : 'text-gray-400 hover:text-gray-600'}`}>
                Chi tiết
                {activeTab === 'details' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-orange-600 rounded-t-full" />}
              </button>
              <button onClick={() => setActiveTab('reviews')} className={`pb-2 text-sm font-bold transition-colors relative ${activeTab === 'reviews' ? 'text-orange-600' : 'text-gray-400 hover:text-gray-600'}`}>
            ...
            {activeTab === 'details' ? (
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-gray-600 text-sm leading-relaxed">{item.description || "Chưa có mô tả."}</p>
                </div>
                ...
                {isLoadingReviews ? (
                  ...
                  <div className="text-center py-8">
                    <div className="bg-white p-6 rounded-xl shadow-sm inline-block mb-4">
                       ...
                       <p className="text-xs text-gray-400 mt-1">Hãy là người đầu tiên thưởng thức và đánh giá!</p>
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    {stats && (
                        <div className="flex items-center gap-4 bg-orange-50 p-4 rounded-xl mb-4">
                            <div className="text-center px-2">
                                <span className="text-3xl font-bold text-orange-600 block">{Number(stats.average_rating).toFixed(1)}</span>
                                <div className="flex gap-0.5 justify-center mt-1">
                                    {renderStars(Math.round(Number(stats.average_rating)))}
                                </div>
                                <span className="text-[10px] text-gray-500 mt-1 block">{stats.total_reviews} đánh giá</span>
                            </div>
                            <div className="h-10 w-[1px] bg-orange-200"></div>
                            <div className="text-xs text-gray-600 flex-1">
                                <p>Được đánh giá cao bởi khách hàng về hương vị và chất lượng phục vụ.</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="space-y-3">
                        {reviews.map((review) => (
                            <div key={review.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 animate-fade-in-up">
                            <div className="flex justify-between items-start mb-2">
                                <div className="flex items-center gap-2">
                                <div className="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 font-bold text-xs">
                                    {review.customer_name ? review.customer_name.charAt(0).toUpperCase() : <User size={16} />}
                                </div>
                                <div>
                                    <p className="text-sm font-bold text-gray-800">{review.customer_name || 'Khách hàng'}</p>
                                    <p className="text-[10px] text-gray-400">
                                        {review.created_at ? new Date(review.created_at).toLocaleDateString('vi-VN') : ''}
                                        {review.is_verified_purchase && <span className="ml-2 text-green-600 bg-green-50 px-1.5 py-0.5 rounded text-[9px] font-semibold">Đã mua</span>}
                                    </p>
                                </div>
                                </div>
                            ...
                            {review.comment && <p className="text-sm text-gray-600 pl-10 mt-1">{review.comment}</p>}
                            
                            {review.admin_response && (
                                <div className="mt-3 ml-10 p-3 bg-gray-50 rounded-lg border-l-2 border-orange-400 text-xs">
                                    <span className="font-bold text-gray-800 block mb-1">Phản hồi từ nhà hàng:</span>
                                    <span className="text-gray-600">{review.admin_response}</span>
                                </div>
                            )}
                            </div>
                        ))}
                    ...
                    {pagination.totalPages >= 1 && (
                        <div className="flex justify-center items-center gap-4 pt-4 border-t border-gray-100 mt-4">
                            ...
                            <span className="text-sm font-medium text-gray-600">
                                Trang <span className="text-orange-600 font-bold">{currentPage}</span> / {pagination.totalPages}
                            </span>
          ...
          <div className="p-4 bg-white border-t shadow-lg">
            <button onClick={() => onAddToOrder(item)} className="w-full py-3.5 bg-gradient-to-r from-orange-500 to-red-600 text-white rounded-xl font-bold shadow-orange-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">
